...
class ChatRoomConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        self.user = self.scope['user']  # The user from the scope
        
        await self.validate_user(self.user.id)
    ...
    def validate_user(self, send_user_id):
        ...
        users_in_room = self.room_name.split('and')
        if str(send_user_id) not in users_in_room:
            raise CustomError(
                message='Access Denied: You are not allowed to join this room',
    ...
    def fill_user_name(self, send_user_id: int):
        ...
        if user.user_type == 'members':
            ...
            user_full_name = f'admin: {user.first_name}'
