...
def sendInvitationMail(user,event:event_models.Event,event_proxy_attendies:event_models.EventProxyAttendies):
    mail_subject = f'Invitation for {event.name}'
    ...
    print({
        'value':event_proxy_attendies.participants,
        'type':type(event_proxy_attendies.participants)
    })
    data ={
    ...
    send_mail(mail_subject,'',os.environ['domain_mail'],recipient_list=to_emails,html_message=message,)
